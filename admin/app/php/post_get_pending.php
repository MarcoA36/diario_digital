...
$mensaje = "SELECT * from noticias where id_estado = 2";
...
if ($ejecutar) {
    if (mysqli_num_rows($ejecutar) > 0) {
        ...
        respuesta(false, 'no hay posts en revision');
    }
} else {
    respuesta(false, 'error en la consulta');
}

$numero_noticias = count($noticias);
mysqli_close($con);

echo json_encode(array('noticias' => $noticias, 'numero_noticias' => $numero_noticias));
